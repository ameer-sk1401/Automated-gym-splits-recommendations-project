...
def list_user_custom_splits(username: str):
    ...
    Order: by filename ascending, so users can control sequence by naming files:
      1-push.json, 2-pull.json, 3-legs.json, ...
    """
    user_dir = WORKOUT_SPLITS / username
...
def pick_today_index(username: str, today_iso: str, total: int) -> int:
    """
    Rotation rule with skip-freeze:
      - If last_action == "SKIPPED" and last_action_date is yesterday or today => DO NOT advance.
      - Else advance by 1 (wrap).
    Set today's baseline to NONE (submit endpoint will set COMPLETED/SKIPPED).
    """
    ...
    sched.update({"current_index": idx, "last_action": "NONE", "last_action_date": today_iso})
...
def pick_split_for_today(username: str, today_iso: str) -> dict:
    """
    Preference order:
      1) If user has custom files: workout_splits/<username>/*.json (sorted by filename)
      2) Else fallback to DEFAULT_ROTATION_TITLES from /splits
    Returns a split dict: { title, exercises: [...] }
    """
    custom_files = list_user_custom_splits(username)
    if custom_files:
        idx = pick_today_index(username, today_iso, total=len(custom_files))
        return load_split_file(custom_files[idx])
